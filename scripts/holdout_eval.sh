...
CR_AGENT_EVAL_FIXTURES="${CR_AGENT_HOLDOUT_FIXTURES:-"holdout-safe-refactor.diff holdout-placeholder-secret.diff holdout-secret-private-key.diff holdout-lifecycle-combo.diff holdout-pr-shaped-service.diff holdout-guarded-lifecycle.diff holdout-batch-worker-combo.diff holdout-env-secret-guard.diff model-semantic.diff model-authz-bypass.diff model-nil-boundary.diff model-state-inconsistency.diff model-transaction-semantic.diff model-error-swallow.diff model-safe-semantic.diff"}" \
