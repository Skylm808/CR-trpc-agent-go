...
`testdata/holdout/` 是提交到仓库的自包含 holdout matrix，不是假装私有 hidden 数据。它用于覆盖 public fixture 之外的误报边界、组合风险和无 API key 的 fake-model 增量路径。
...
## Hidden / External 样本与评测

Issue 要求隐藏/holdout 样本检出率 ≥ 80%、误报 ≤ 15%。Issue reviewer 当前没有提供私有 hidden 样本，因此仓库提交 `testdata/holdout/` 作为 self-contained holdout/adversarial matrix，并用 `scripts/hidden_matrix_smoke.sh` 证明 external matrix 注入机制。它们是可复现替代证据，不伪装成 reviewer 私有 hidden。若未来 reviewer/CI 另有私有样本，`scripts/eval.sh` 仍支持外部 fixture root、external expected TSV、阈值门禁和报告保留目录：
