...
Issue 要求隐藏/holdout 样本检出率 ≥ 80%、误报 ≤ 15%。当前仓库提交 `testdata/holdout/` 作为 self-contained holdout matrix；如 reviewer/CI 另有私有 hidden 样本，`scripts/eval.sh` 也支持外部 fixture root、external expected TSV、阈值门禁和报告保留目录：
