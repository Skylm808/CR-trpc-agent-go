...
生产与 CI 默认使用 `codeexecutor/container` 或 E2B runtime，本地 fallback 只能通过 `--runtime local-fallback` 显式启用。沙箱执行前必须经过 `tool.PermissionPolicy` 或兼容 wrapper 决策：`deny`、`ask`、`needs_human_review` 的命令不进入 executor。执行控制包括超时（默认 30s）、stdout/stderr 输出大小上限、环境变量白名单和 artifact 记录。stdout/stderr 以 digest 形式落库，明文密钥在写入前脱敏。沙箱超时或命令失败记录为 `status=timed_out/failed`，不导致整个 review 任务崩溃。
