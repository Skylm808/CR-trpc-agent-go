...
HTTP provider 通过 `--model-provider http`、`--model-endpoint`、`--model-api-key-env`、`--model-name` 显式开启。OpenAI-compatible / DeepSeek 通过 `--model-provider openai|openai-compatible|deepseek`、`--model-name`、`--model-api-key-env`、`--model-base-url`、`--model-variant` 显式开启；DeepSeek 默认使用官方 `model/openai` 的 `VariantDeepSeek` 和 `DEEPSEEK_API_KEY`。API key 只从 env 读取，不进入报告、diagnostics、SQLite 或 telemetry。HTTP 请求失败、非 2xx、JSON 解析失败、deadline/timeout 或缺少 API key 都会降级为人工复核。Claude / Gemini 厂商 SDK 绑定、Session/Memory 和真实 E2B adapter 都不属于当前阶段。
...
- `cr-agent.yaml`：默认自动读取当前目录配置；`--config` 可显式指定，CLI flags 覆盖 YAML。
