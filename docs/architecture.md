...
- `internal/semantics` 模型审查边界、无 API Key 的 fake provider、显式 opt-in HTTP / OpenAI-compatible / DeepSeek provider，以及官方 `model.Model` adapter
...
当前模型审查边界在 `internal/semantics`，支持默认 fake provider、generic HTTP provider，以及官方 `trpc-agent-go/model/openai` 的 OpenAI-compatible / DeepSeek provider。`semantics.Input` 的输入只包含脱敏后的 diff summary、Go input metadata、已有 findings、sandbox summary 和 governance summary；raw diff secret 不允许进入 prompt。运行时通过官方 `model.Model.GenerateContent(ctx,*model.Request)` 获取结构化 JSON，再映射回 `semantics.Output`。Provider 输出继续使用 `review.Finding` 字段：`severity`、`category`、`file`、`line`、`title`、`evidence`、`recommendation`、`confidence`、`source`、`rule_id`、`status`。
