...
- 在 `review` 模式下通过独立的 `model.enabled` 开关进入 LLM Review Provider；默认 deterministic fake provider，显式选择外部 provider 时才联网。
...
- `internal/llm` 模型审查边界、无 API Key 的 fake provider、显式 opt-in HTTP / OpenAI-compatible / DeepSeek provider，以及官方 `model.Model` adapter
- `internal/execution` 执行器/runtime 边界、workspace Go checks、timeout、output limit 和实际 env allowlist
- `internal/approval` 固定命令 allowlist 和 `tool.PermissionPolicy`
...
CLI 输入（--diff-file / --file-list / --repo-path / --fixture；未传输入时推断 --repo-path .）
  ...
  -> 可选 sandbox capability：internal/execution 优先 tool/workspaceexec 执行 go test / go vet / staticcheck，失败时退回 tool/codeexec
  -> 可选 model capability：redacted input + sandbox/governance summary -> internal/llm -> provider(fake/http/openai/deepseek) -> finding/warning merge
  -> 通过 official event.Event 输出 input/skill/model/sandbox/report/task 阶段事件
  -> 合并 Skill 输出，去重，脱敏，分流 warnings / human_review_items
  -> 生成 review_report.json、review_report.md 和 review_report.zh.md
...
| Model Review Provider | `internal/llm.Provider` 接收脱敏 diff 摘要、input metadata、existing findings、sandbox/governance summary，输出复用 `review.Finding`；默认 fake provider，显式 opt-in HTTP provider 使用标准库 `net/http`，OpenAI-compatible / DeepSeek 使用官方 `model/openai`；`internal/agent` 只选择 provider 并触发 review step | ✅ 已通过官方 `trpc-agent-go/model.Model` 调用路径，无 API Key 默认路径保持可测 |
| CodeExecutor | `internal/execution` 创建 `codeexecutor/container` 默认执行器，`codeexecutor/local` 仅显式 fallback；`e2b` 当前返回 unsupported audit，不执行本地 fallback | ✅ Issue 主线由默认 container 满足；E2B/Cube 是远端沙箱扩展 |
| PermissionPolicy | `internal/approval.NewPermissionPolicy` 返回 `tool.PermissionPolicy`，所有 Skill/Go check 命令复用 `AllowedReviewCommands` allowlist | ✅ 固定 allowlist，非 allow 不进入 executor |
...
| Artifact | `review_report.json` / `review_report.md` / `review_report.zh.md` / `review_diagnostics.json` 保存到本地，且同步写入官方 artifact service | ✅ 默认使用官方 inmemory service，SQLite 继续保留引用记录 |

## CLI Mode

| Mode | 行为 | 用途 |
|------|------|------|
| `review` | 始终运行 deterministic Skill；`sandbox.enabled` 和 `model.enabled` 可独立组合 | 默认与生产评审路径 |
| `dry-run` | 加载 Skill，记录 skipped decision/run，不进入代码执行器或模型 Provider | 验证治理、持久化与报告链路 |

兼容映射：`rule-only -> review`，`sandbox -> review+sandbox`，
`fake-model -> review+model`。新任务只持久化 canonical mode，并在 metrics 中记录
`sandbox_requested/executed` 与 `model_requested/executed`。

## LLM Review Provider

当前模型审查边界在 `internal/llm`，支持默认 fake provider、generic HTTP provider，以及官方 `trpc-agent-go/model/openai` 的 OpenAI-compatible / DeepSeek provider。`llm.Input` 的输入只包含脱敏后的 diff summary、Go input metadata、已有 findings、sandbox summary 和 governance summary；raw diff secret 不允许进入 prompt。运行时通过官方 `model.Model.GenerateContent(ctx,*model.Request)` 获取结构化 JSON，再映射回 `llm.Output`。Provider 输出继续使用 `review.Finding` 字段：`severity`、`category`、`file`、`line`、`title`、`evidence`、`recommendation`、`confidence`、`source`、`rule_id`、`status`。
...
CLI Request
  ...
  -> model.Model adapter(redacted llm.Input)
...
Permission 负责“命令能不能执行”，统一在 `internal/approval` 定义：
...
container 模式下 Go 检查先经过 `tool.PermissionPolicy`，再通过 `internal/execution` 构造官方 `tool/workspaceexec` 调用进入容器 workspace。Agent 记录的审计命令保持为 `go test ./...` / `go vet ./...`，容器内实际执行使用 `/usr/local/go/bin/go`，避免治理策略清理 PATH 后找不到 Go 工具链。

`internal/execution.SandboxEnv` 是实际执行 env 边界，不只是审计字段。当前只向 workspace command 传 `PATH`、`HOME`、`TMPDIR`、`GOCACHE`，不传 `OPENAI_API_KEY`、`DEEPSEEK_API_KEY` 或其他 provider secret env。报告/SQLite 中的 `env_whitelist` 必须和该实际 allowlist 对齐。
