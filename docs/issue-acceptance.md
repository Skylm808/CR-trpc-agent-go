...
本文档面向最终验收：把 Issue 要求映射到可运行命令、代码路径、测试和 sample 输出。当前实现是基于 `trpc-agent-go` Tool / Skill / CodeExecutor / PermissionPolicy / artifact / telemetry / SQLite 的 CLI Agent 原型；已接入 LLM Review Provider 边界和无 API Key fake provider，尚未绑定真实模型厂商，也尚未接入 Runner/Event、Session/Memory 和 E2B runtime。
...
| LLM provider boundary | `fake-model` 模式在 Skill 后调用 `ModelReviewProvider`，输入/输出脱敏，失败降级人工复核 | `TestAgentRunFakeModelUsesProviderBoundary`、model provider tests |
...
- 真实 LLM provider：当前只是 provider 边界和 fake provider，不接 OpenAI/Claude/Gemini SDK，不需要 API Key。真实 provider 后续接入同一边界。
...
- 当前 fake provider 只验证模型审查链路、分流、脱敏、审计和失败降级；复杂业务逻辑错误仍需后续真实模型 provider 或更多领域规则补齐。
