...
本文档面向最终验收：把 Issue 要求映射到可运行命令、代码路径、测试和 sample 输出。当前实现是基于 `trpc-agent-go` Tool / Skill / CodeExecutor / PermissionPolicy / artifact / telemetry / SQLite 的 CLI Agent 原型；已接入 LLM Review Provider 边界、无 API Key fake provider 和显式 opt-in generic HTTP provider，尚未绑定真实模型厂商 SDK，也尚未接入 Runner/Event、Session/Memory 和 E2B runtime。
...
- 厂商 SDK provider：当前已有 provider 边界、fake provider 和 opt-in HTTP provider，但不接 OpenAI/Claude/Gemini SDK，不要求 API Key。厂商特化 provider 后续可接入同一边界。
...
- 当前 fake provider 和 HTTP provider 测试验证模型审查链路、分流、脱敏、审计和失败降级；复杂业务逻辑错误的真实检出率仍取决于外部模型端点或更多领域规则。
