...
CI 配置见 `.github/workflows/acceptance.yml` 和 [ci.md](ci.md)。
...
| artifact | `review_report.json`、`review_report.md`、`review_diagnostics.json` 同步为 artifact 记录，可接官方 artifact service | `TestAgentRunPersistsArtifactsThroughOfficialService` |
...
`testdata/fixtures/` 当前有 14 条公开样本，覆盖无问题、安全、敏感信息脱敏、panic、TODO、测试缺失、goroutine/context/resource/db 生命周期、重复 finding、沙箱失败和超时。`cmd/review-agent/fixtures_test.go` 会逐条运行并校验 rule_id、severity、status 和脱敏结果；`scripts/eval.sh` 输出公开样本 recall、precision 和 false positive rate。

隐藏样本通过 `CR_AGENT_EVAL_FIXTURES_ROOT`、`CR_AGENT_EVAL_FIXTURES` 和 `CR_AGENT_EVAL_EXPECTED` 注入；`scripts/eval_test.go` 固定验证外部 expected TSV 路径。验收阈值按 Issue 要求执行：`recall >= 0.800` 且 `false_positive_rate <= 0.150`。
...
- 隐藏样本的 80% 检出率和 15% 误报率需要外部 expected matrix 持续校准。
- Docker container E2E 依赖本机或 CI Docker daemon；CI 中通过 `CR_AGENT_RUN_CONTAINER_E2E=1` 启用独立 job。
