...
2. 补齐真实沙箱验证：运行或维护 container runtime integration test（默认跳过，设置环境变量才跑 Docker），保持 Agent 层 ask/deny/needs_human_review 不进入 executor 的测试，确保 go test/go vet/staticcheck 都有 permission decision 和 sandbox run。
