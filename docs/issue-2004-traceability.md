...
2. E2B/Cube 真实 runtime adapter 未接入；当前只有 `--runtime e2b` unsupported 审计入口，且测试固定它不会静默 fallback 到 local/container。最小 adapter 仍需明确 API/env 配置、workspace staging、artifact 拉取和 cleanup contract。
3. hidden matrix 支持外部注入，并有 hidden-like smoke 证明入口；真实 hidden 数据未随仓库提交，本机也没有外部 hidden fixture root / expected matrix，因此未能证明真实 hidden 达标。
...
1. 接入真实 hidden fixture root / expected matrix 后运行 `scripts/eval.sh`，记录 recall、precision、false_positive_rate 和 report_root。
2. 若 Issue reviewer 明确要求 E2B/Cube，先实现最小 adapter 的 staging/cleanup contract，再替换 unsupported 入口。
