...
| 4 | 输入解析（diff / 文件列表 / git 变更 / base-head / 当前目录默认推断） | `cmd/review-agent.Run`、`internal/agent.readInput`、`internal/agent.inputMetadata`、`internal/review/parser.go` | `parser_test.go`、`repo_test.go`、`agent_test.go`、CLI base/head test | ✅ | 不自动 fetch 远端 ref |
