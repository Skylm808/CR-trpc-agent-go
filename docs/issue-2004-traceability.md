...
GOCACHE=/private/tmp/cr-agent-gocache go vet ./...
GOCACHE=/private/tmp/cr-agent-gocache go test -tags=integration -p 1 ./internal/agent ./cmd/review-agent ./scripts
...
| 项目 | 结果 |
|------|------|
| 公开 fixture eval | 连续 3 轮均为 `fixtures=21`、`expected=28`、`TP=28`、`FP=0`、`FN=0`、`recall=1.000`、`precision=1.000`、`false_positive_rate=0.000`、`infrastructure_failures=0`；耗时 39s / 38s / 38s |
| self-contained holdout matrix | `fixtures=17`、`expected=31`、`TP=31`、`FP=0`、`FN=0`、`recall=1.000`、`false_positive_rate=0.000`、`infrastructure_failures=0`、`duration_ms=31000` |
...
| Go verification | 快速层 `go test ./...` 约 17.2s；最终 acceptance 中 integration 的 Agent/CLI 为 59.4s/35.8s（scripts 命中缓存，独立实测 16.9s）。快速 race 18.0s；integration race 为 61.1s/36.8s/20.6s。完整 fixture/holdout 由独立评测脚本负责，避免 Go 测试重复运行矩阵 |
| container E2E | `CR_AGENT_RUN_CONTAINER_TESTS=1 ... TestAgentRunContainerRuntimeExecutesGoChecks` 通过（5.801s）；同时验证 container Skill secret finding、`go test` 和 `go vet` |
| 无密钥回放 | `dry-run` 与 `review + model.enabled + provider=fake` 均生成四份报告和默认 `review.db`；报告与 SQLite dump 明文密钥扫描为空 |
...
| 3 | skill_run / workspace_exec / PermissionPolicy | `tool/skill`、`tool/workspaceexec`、`tool/codeexec`、`tool.PermissionPolicy` | `agent_test.go`、`policy_test.go` | ✅ | workspace 失败后的 codeexec fallback 使用实际 Tool/Arguments 再审批并单独落库 |
...
| 6 | 数据库存储 | `internal/storage`、`internal/storage/sqlite` | `sqlite_test.go`、`agent_test.go` | ✅ | CLI 默认写 `output_dir/review.db`；领域类型与 SQLite 解耦；最终审计通过事务原子提交并有 FK/index |
| 7 | 去重降噪 | `DedupeFindings`、`dedupe.diff` | `types_test.go`、fixture tests | ✅ | 使用 `file + line + category + rule_id`；同文件不同行同类问题保留 |
| 8 | 安全边界 | `internal/execution` timeout/流式 output limit/env allowlist/digest、Agent redaction、artifact 名称/数量/单体/总量限制 | `sandbox-safety.md` + sandbox failure/timeout tests + 多形态 secret 报告/DB 扫描 + container E2E | ✅ | 上游 staging 当前不兼容应用层 non-root 与 read-only-rootfs + tmpfs，部署约束见 `sandbox-safety.md` |
...
| workspace Go 检查编排 | `review + sandbox.enabled` 执行 `go test ./...`、`go vet ./...`、可选 `staticcheck ./...`，并记录 permission/sandbox run | `internal/agent/sandbox_step.go`、`internal/execution` |
...
| Permission 治理 | 所有 `skill_run` / workspace Go check / codeexec fallback 都按实际调用分别经过 `tool.PermissionPolicy`，非 allow 不执行 | `internal/approval`、`TestRunGoSandboxCommandDoesNotCallFallbackWhenSecondDecisionAsks` |
...
| LLM provider boundary | `review + model.enabled` 在 Skill/沙箱后经 `internal/llm` 和官方 `model.Model` 调用模型边界；默认 fake provider，显式选择外部 provider 才联网。输入/输出脱敏，失败降级人工复核 | `TestAgentRunFakeModelUsesProviderBoundary`、`TestReviewCanCombineSandboxAndModel`、HTTP/openai/model provider tests |
...
| SQLite 审计 | task、finding、permission/filter decision、sandbox run、artifact、metrics、report 按 task id 查询；完整结果原子提交 | `TestAcceptanceEvidenceReportsAndSQLiteReplay`、`TestSaveReviewRollsBackEveryRecordOnFailure` |
...
| review / dry-run | 两个 canonical mode；sandbox/model 为独立 capability，旧模式仅作兼容输入 | ✅ |
...
| 8+ 测试样例 | `testdata/fixtures/*.diff`、`testdata/holdout/*.diff` | ✅ public fixtures 23 个（accuracy matrix 21 个，故障注入 2 个）+ holdout 17 个 |
...
| # | 验收标准 | 状态 | 验证方式 | 缺口 |
|---|---------|------|---------|------|
| 1 | 8 条公开 diff 全部可运行并生成报告 | ✅ | `TestAllFixturesMatchExpectedReviewResults` 覆盖 23 条 fixture；accuracy matrix 排除 2 条专门故障注入样本后稳定运行 21 条 | — |
...
| 6 | dry-run/fake-provider 全流程 <= 2 分钟 | ✅ | unit/integration tests | — |
| 7 | 高风险命令须先过 Filter/Permission；非 allow 不进沙箱 | ✅ | `policy_test.go` + workspace/fallback 独立 ask/deny E2E | — |
| 8 | 报告含 findings 摘要、severity 统计、人工复核项、治理拦截、监控、沙箱摘要、修复建议和 conclusion | ✅ | committed container 示例含 8 findings、2 warnings、1 human review、3 permission decisions、`go test` failed、`go vet` ok 和 `sandbox_failed=1` | — |
...
当前没有阻塞 Issue #2004 功能验收的已知缺口。生产多租户隔离仍有一个明确的上游限制：`codeexecutor/container` 的 root-owned `CopyToContainer` staging 与 `/tmp` metadata 回放当前不兼容应用层强制 non-root 或 read-only-rootfs + 自定义 tmpfs。已验证的应用层边界包括禁网、capability drop、no-new-privileges、PID/CPU/内存限制、命令前审批、超时、流式输出限制和脱敏；更强 UID/rootfs 隔离需由支持映射的外层 runtime 提供。
