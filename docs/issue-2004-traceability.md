...
| self-contained holdout matrix | `fixtures=11`、`expected=10`、`matrix_source=holdout`、覆盖 false-positive guardrail、组合生命周期风险和 6 类 fake-model 增量 finding |
...
| 8 | 安全边界 | Agent timeout/output limit/digest/redaction、artifact size/cap、env whitelist audit | `sandbox-safety.md` + sandbox failure/timeout tests + 多形态 secret 报告/DB 扫描 | ✅ | 生产部署可继续做平台级 runtime 加固 |
...
2. `scripts/check.sh` 仍是单文件规则实现；后续可在保持入口稳定的前提下拆成 helper scripts。
