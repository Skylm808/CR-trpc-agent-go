...
| 外部 hidden fixture matrix | reviewer 未提供私有 hidden 样本时不作为 blocker；仓库保留 `CR_AGENT_EVAL_FIXTURES_ROOT` / `CR_AGENT_EVAL_FIXTURES` / `CR_AGENT_EVAL_MATRIX` 注入契约，并用 hidden-like smoke 验证 |
...
| 真实 repo LLM go-only | `/Users/skylm/Desktop/GOLAND/trpc-agent/trpc-GitHub-agent` 使用 `scripts/repo_llm_smoke.sh --go-only` 通过；报告含 `model_provider=deepseek`、`model_name=deepseek-chat`、`model_backend=trpc-agent-go/model/openai`、`model_call_count=1`，无 key 泄漏，且无 `model-provider-failed`；本轮模型产生 `source=model` 增量 finding，rule_id 与 deterministic `skill_run` 规则分离 |
...
| 2 | 隐藏/holdout 样本高危检出率 >= 80%，误报率 <= 15% | ✅ | `scripts/holdout_eval.sh` 提供 self-contained holdout/adversarial matrix；`scripts/eval.sh` 继续支持 external expected TSV、阈值门禁和报告保留；`scripts/hidden_matrix_smoke.sh` 证明外部注入 contract | reviewer 未提供私有 hidden 样本时不阻塞；仓库内 holdout 不伪装成 reviewer hidden |
...
2. 继续扩充 self-contained holdout/adversarial fixtures，模拟 reviewer 未提供私有 hidden 时的可复现替代证据；外部 hidden 注入契约保留但不作为 blocker。
...
2. 如 Issue reviewer 未来明确提供私有 hidden fixtures，运行 `scripts/eval.sh` 并记录 recall、precision、false_positive_rate 和 report_root；未提供时使用仓库自包含 holdout 和 hidden-like smoke。
