...
本文档将 Issue #2004 的能力要求、输入输出要求、交付物、验收命令和剩余缺口映射到当前仓库实现。当前实现是基于 `trpc-agent-go` Tool / Skill / CodeExecutor / PermissionPolicy / artifact / telemetry / SQLite 的 CLI Agent 原型；已接入 LLM Review Provider 边界、无 API Key fake provider、显式 opt-in generic HTTP provider、官方 `model.Model` 薄 adapter 和官方 `event.Event` 阶段事件 facade。尚未接入完整官方 Runner 托管、Session/Memory、E2B runtime 和 base/head ref 输入。
...
| 5 | 结构化 findings | `internal/review/types.go`、`internal/agent/model.go`、`internal/agent/model_http.go`、`internal/agent/model_official.go` | `engine_test.go`、fixture tests、model provider tests | ✅ | provider 已走官方 `model.Model` adapter；完整 Runner 托管仍未接入 |
...
| 官方模型路线 | 已实现 `trpc-agent-go/model.Model` adapter；已通过 `event.Event` facade 暴露模型和任务阶段事件；完整 Runner 托管未接入 | 🔶 |
...
1. 完整官方 Runner 托管未接入；当前是官方 `model.Model` adapter + `event.Event` facade + CLI `Agent.Run`。
...
1. 将当前 `event.Event` facade 收敛到完整官方 Runner 路线。
