...
| 真实模型调用链路 | 已有 opt-in generic HTTP provider 和官方 `model/openai` OpenAI-compatible / DeepSeek provider；`testdata/holdout/model-*.diff` 用 fake provider 稳定证明 authz、nil boundary、state inconsistency、transaction semantic、error swallow 等模型增量合并路径；真实语义质量需持续用 DeepSeek/OpenAI smoke 校准 | ✅ 调用链路完成 |
