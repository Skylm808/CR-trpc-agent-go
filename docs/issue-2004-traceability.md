...
| self-contained holdout matrix | `fixtures=5`、`matrix_source=holdout`、覆盖 false-positive guardrail、组合生命周期风险和 fake-model 增量 finding |
...
| 真实模型语义能力 | 已有 opt-in generic HTTP provider 和官方 `model/openai` OpenAI-compatible / DeepSeek provider；`testdata/holdout/model-semantic.diff` 用 fake provider 稳定证明模型增量合并路径；真实效果取决于外部 endpoint/model | ✅ |
...
1. 持续扩充 holdout/adversarial 样本，特别是真实模型能稳定补充的跨函数/业务语义风险。
