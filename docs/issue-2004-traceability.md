...
| `cr-agent.yaml` / `--config` | 默认只读取当前目录本地 `cr-agent.yaml`，显式 `--config` 可指定路径，CLI flags 覆盖 YAML；根目录本地 YAML 被 gitignore 忽略，提交 `cr-agent.example.yaml`；`api_key_env` 是环境变量名，`api_key` 只用于本机 smoke/debug | ✅ |
...
| 真实 LLM smoke | `scripts/llm_smoke.sh` 支持 env 参数和 `CR_AGENT_LLM_CONFIG`，opt-in 后生成报告并校验 `model_call_count=1`，同时检查 report/diagnostics 不泄漏 key 或 key env name | ✅ |
