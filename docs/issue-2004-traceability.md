...
外部 hidden 样本可通过 root 和 expected TSV 注入，不随仓库提交：
...
| self-contained holdout matrix | `fixtures=15`、`expected=22`、`matrix_source=holdout`、覆盖 multi-file PR-shaped diff、false-positive guardrails、组合生命周期风险和 6 类 fake-model 增量 finding |
| 外部 hidden fixture matrix | 支持 `CR_AGENT_EVAL_FIXTURES_ROOT` / `CR_AGENT_EVAL_FIXTURES` / `CR_AGENT_EVAL_MATRIX` 注入；如 reviewer/CI 提供私有样本可额外运行 |
...
| Issue 能力 | 当前实现 | 证据 |
|------------|----------|------|
| Skill 加载与执行 | `tool/skill` 的 `skill_load` / `skill_run` 执行 `skills/code-review/scripts/check.sh`；入口脚本分发到 `check_rules.py`，无 Python 时走 `check_fallback.go` | `internal/agent/execution.go`、`skills/code-review/SKILL.md`、`internal/review/skill_test.go` |
...
| 8+ 测试样例 | `testdata/fixtures/*.diff`、`testdata/holdout/*.diff` | ✅ public 15 个 + holdout 15 个 |
...
| 2 | 隐藏/holdout 样本高危检出率 >= 80%，误报率 <= 15% | ✅ | `scripts/holdout_eval.sh` 提供 self-contained holdout matrix；`scripts/eval.sh` 继续支持 external expected TSV、阈值门禁和报告保留；`scripts/hidden_matrix_smoke.sh` 证明外部注入 contract | 私有 hidden 样本如由 reviewer/CI 提供，可额外运行；不作为本仓库 blocker |
...
| 真实模型语义能力 | 已有 opt-in generic HTTP provider 和官方 `model/openai` OpenAI-compatible / DeepSeek provider；`testdata/holdout/model-*.diff` 用 fake provider 稳定证明 authz、nil boundary、state inconsistency、transaction semantic、error swallow 等模型增量合并路径；真实效果取决于外部 endpoint/model | ✅ |
...
1. 持续用真实 DeepSeek/OpenAI smoke 校准 semantic holdout 的提示有效性，记录模型是否产生非重复增量 finding。
2. 如果 reviewer/CI 提供私有 hidden fixtures，运行 external matrix 并记录 recall、precision、false_positive_rate 和 report_root；仓库内 holdout/adversarial fixtures 不能冒充真实 hidden matrix。
...
2. 如 Issue reviewer 明确提供私有 hidden fixtures，运行 `scripts/eval.sh` 并记录 recall、precision、false_positive_rate 和 report_root。
