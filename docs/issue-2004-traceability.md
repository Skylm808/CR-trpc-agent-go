...
本文档将 Issue #2004 的能力要求、输入输出要求、交付物、验收命令和剩余缺口映射到当前仓库实现。当前实现是基于 `trpc-agent-go` Tool / Skill / CodeExecutor / PermissionPolicy / artifact / telemetry / SQLite 的 CLI Agent 原型；已接入 LLM Review Provider 边界、无 API Key fake provider 和显式 opt-in generic HTTP provider。尚未接入官方 `model.Model` / Runner / Event、Session/Memory、E2B runtime 和 base/head ref 输入。
...
| 2 | 沙箱执行（container/E2B，local 仅 fallback） | `codeexecutor/container`、`tool/workspaceexec`、`tool/codeexec` | workspaceexec 主路径/fallback tests + env-gated Docker test | 🔶 | container E2E 已通过；E2B/Cube runtime 未接入 |
...
| 5 | 结构化 findings | `internal/review/types.go`、`internal/agent/model.go`、`internal/agent/model_http.go` | `engine_test.go`、fixture tests、model provider tests | 🔶 | 当前是自定义 `ModelReviewProvider`；官方 `trpc-agent-go/model.Model` / Runner/Event 未接入 |
...
| LLM provider boundary | `fake-model` 模式在 Skill 后调用 `ModelReviewProvider`，默认 fake provider；显式 `--model-provider http` 调用 HTTP provider。输入/输出脱敏，失败降级人工复核 | `TestAgentRunFakeModelUsesProviderBoundary`、HTTP/model provider tests |
...
| 官方模型路线 | 尚未实现 `trpc-agent-go/model.Model` adapter，也尚未通过 Runner/Event 暴露模型调用事件 | ⬜ |
...
1. 官方 `model.Model` / Runner / Event 未接入；当前是自定义 `ModelReviewProvider` + CLI `Agent.Run`。
2. 官方 `session/sqlite` / Memory 未接入；当前 SQLite 是审计 store，不是会话服务。
3. E2B/Cube runtime 未接入；container 是默认 runtime，local fallback 只能显式选择。
4. base/head ref 未支持；`--repo-path` 当前读取 working tree diff 或目录内容。
5. hidden matrix 支持外部注入，但真实 hidden 数据未随仓库提交，也未在公开仓库内证明达标。
6. 官方 metric exporter / OTLP dashboard 未接；当前只有 trace span + SQLite metrics。
7. runtime 级 env 强隔离仍依赖部署侧 executor 配置；当前仓库记录 env whitelist 审计字段。

## 下一步

1. 把 LLM provider 适配到官方 `trpc-agent-go/model.Model` / Runner / Event 路线。
